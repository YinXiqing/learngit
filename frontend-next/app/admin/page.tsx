  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f0f0f] py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8"><h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">管理后台</h1><p className="text-gray-600 dark:text-gray-400 mt-1">平台数据概览和管理</p></div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {cards.map(c => (
            <Link key={c.title} href={c.href} className="bg-white dark:bg-[#1f1f1f] rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow flex items-center">
              ...
              <div><p className="text-sm text-gray-500 dark:text-gray-400">{c.title}</p><p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{loading ? '-' : c.value.toLocaleString()}</p></div>
            </Link>
          ))}
        </div>

        <div className="bg-white dark:bg-[#1f1f1f] rounded-xl shadow-sm p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <Link key={item.href} href={item.href} className={`flex items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg transition-colors ${item.color}`}>
        ...
        <div className="bg-white dark:bg-[#1f1f1f] rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">待审核视频</h2>
              ...
              {recentVideos.map(v => (
                <div key={v.id} className="py-4 flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 dark:text-gray-100 truncate">{v.title}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{v.author} • {new Date(v.created_at).toLocaleDateString()}</p>
                  </div>
          ...
          ) : <p className="text-gray-500 dark:text-gray-400 text-center py-4">暂无待审核视频</p>}
        </div>
      </div>
    </div>
    </RequireAdmin>
